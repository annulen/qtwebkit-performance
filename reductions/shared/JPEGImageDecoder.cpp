...
public:
        ...
        , m_samples(0)
    {
        ...
        if (!m_info.src) {
            src = (decoder_source_mgr*)calloc(sizeof(decoder_source_mgr), 1);
            if (!src) {
                m_state = JPEG_ERROR;
                return;
            }
    ...
    bool decode(const QByteArray& data, bool sizeOnly) {
...
void JPEGImageDecoder::setData(QByteArray* data, bool allDataReceived)
{
    if (m_failed)
        return;
